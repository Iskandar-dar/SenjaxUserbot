    ...
    if HEROKU_API_KEY is not None:
        ...
        if HEROKU_APP_NAME is None:
            await edit_or_reply(event,
                                ...
                                " untuk dapat deploy perubahan terbaru dari ⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡.`"
                                )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            await edit_delete(event,
                              f"{txt}\n`Kredensial Heroku tidak valid untuk deploy Senja-Project dyno.`"
                              )
            return repo.__del__()
        await edit_or_reply(event,
        ...
        if build.status == "failed":
            ...
            await edit_delete(event,
                              "`SenjaxUserbot Berhasil DiUpdate✅,Restart Tunggu Sebentar`"
                              )

        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, "#BOT \n" "`SenjaxUserbot Berhasil Di Update`"
            )
...
async def update(event, repo, ups_rem, ac_br):
    ...
    x = await edit_or_reply(event, "**⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡** `Berhasil Di Update!`")
    await asyncio.sleep(1)
    await x.edit("**⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡** `Di Restart....`")
    await asyncio.sleep(1)
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n" "**⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡ Telah Di Perbarui.**"
        )
...
@Xa_cmd(pattern="update(?: |$)(now|deploy)?")
...
async def upstream(event):
    ...
    if changelog == "" and force_update is False:
        await xx.edit(
            f"\n⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡ Sudah Versi Terbaru || Tunggu Update Terbaru\n"
        )
    ...
    if conf is None and force_update is False:
        changelog_str = (
            f"**Pembaruan Untuk ⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡ :\n\n👨‍💻 Pembaruan Data :**\n`{changelog}`"
        )
    ...
    if force_update:
        ...
        await xx.edit("` Proses Update ⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡, Loading....1%`")
        await xx.edit("` Proses Update ⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡ Loading....20%`")
        await xx.edit("` Proses Update ⚡sᴇɴᴊᴀxᴜsᴇʀʙᴏᴛ⚡, Loading....35%`")
