...
async def _(Nja):
    await Nja.reply(random.choice(xaown))


@register(incoming=True, from_users=1224143544, pattern=r"^Njatest$")
async def _(Nja):
    await Nja.reply(random.choice(njaaalive))
...
async def redis(pong):
    ...
    await pong.edit(
        f"**{ALIVE_NAME}**        \n"
        f"**⌬ Kecepatan : ** '%sms'  \n"
        f"**⌬ Branch : ** 'SenjaxUserbot` \n" % (duration)
    )


@Xa_cmd(pattern="lping$")
async def redis(pong):
    """For .ping command, ping the userbot from any chat."""
...
@Xa_cmd(pattern="xping$")
async def redis(pong):
    """For .ping command, ping the userbot from any chat."""
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("__XXXX.__")
    await pong.edit("__XXXX..__")
    await pong.edit("__XXXX...__")
    await pong.edit("__XXXX.__")
    await pong.edit("__XXXX..__")
...
CMD_HELP.update(
    {
        "ping": f"𝙋𝙚𝙧𝙞𝙣𝙩𝙖𝙝: `{cmd}ping` | `{cmd}lping` | `{cmd}xping` | `{cmd}sinyal` | `{cmd}sping` | `{cmd}pink` | `{cmd}pung`\
         \n⌬𝙁𝙪𝙣𝙜𝙨𝙞 : Untuk Menunjukkan Ping Bot Anda.\
         \n\n𝙋𝙚𝙧𝙞𝙣𝙩𝙖𝙝: `{cmd}kecepatan`\
         \n⌬𝙁𝙪𝙣𝙜𝙨𝙞 : Untuk Menunjukkan Kecepatan Jaringan Anda.\
         \n\n𝙋𝙚𝙧𝙞𝙣𝙩𝙖𝙝: `{cmd}pong`\
         \n⌬𝙁𝙪𝙣𝙜𝙨𝙞 : Sama Seperti Perintah Ping."
    }
)
